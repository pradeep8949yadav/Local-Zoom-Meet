...
# --- Mu-law conversion functions (Identical) ---
_MU = 255.0
...
def safe_serialize(payload):
    try:
        ...
        print(f"Safe-serialize error: {e}. Payload: {payload.get('type', 'N/A')}")
        return b"{}"
            ...
            if key in payload and isinstance(payload[key], str):
                try:
                    ...
                    if "Invalid base64-encoded string" not in str(e):
                        print(f"Could not decode base64 for key '{key}': {e}")
                    pass
        return payload
    except (json.JSONDecodeError, UnicodeDecodeError) as e:
        print(f"Safe-deserialize error: {e}. Data: {data[:50]}...")
        return {}
...
AVATAR_COLORS = ["#F28B82", "#FCCB71", "#A8DAB5", "#8AB4F8", "#C58AFB", "#FDCFE8", "#E6C9A8", "#A142F4"]
                        ...
                        with details['decode_lock']:
                            ...
                            if decoder and label:
                                try:
                                    ...
                                    for frame in frames:
                                        q_pixmap = self.av_frame_to_qpixmap(frame, label)
                                        self.signals.signal_update_video.emit(label, q_pixmap)
                                        
                                # --- *** Catch generic av.AVError *** ---
                                except av.error.AVError:
                                    print(f"UDP video decode error for {sender}, dropping packet.")
                                    # pass # Corrupt packet, just drop it
                                    continue
                    
            except (pickle.UnpicklingError, KeyError, IndexError): pass
                ...
                while self.screen_sharing_active.is_set() and self.is_connected.is_set():
                    try:
                        ...
                        if current_size != last_size or screen_encoder is None:
                            ...
                            # --- *** Use 'zerolatency' tune, not 'screen' *** ---
                            # --- *** Add gop_size for keyframes *** ---
                            screen_encoder.options = {
                                "preset": "ultrafast", 
                                "tune": "zerolatency", 
                                "crf": "25",
                                "gop": "30" # Add a keyframe every 30 frames
                            }
    ...
    def start_camera(self):
        ...
        with self._camera_lock:
            ...
            try:
                ...
                self.video_encoder.width = 640
                self.video_encoder.height = 480
