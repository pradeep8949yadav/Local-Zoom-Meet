...
from auth_utils2 import generate_deterministic_numeric_code, get_lan_ip
...
# --- Mu-law conversion functions ---
_MU = 255.0  # Mu-law constant

def lin2ulaw_numpy(audio_data):
    """Converts linear PCM (16-bit) numpy array to mu-law bytes."""
    # Ensure data is float and scale to [-1.0, 1.0]
    pcm_float = audio_data.astype(np.float32) / 32768.0
    # Apply mu-law compression formula
    compressed = np.sign(pcm_float) * (np.log(1.0 + _MU * np.abs(pcm_float)) / np.log(1.0 + _MU))
...
def ulaw2lin_numpy(ulaw_data):
    """Converts mu-law bytes to linear PCM (16-bit) numpy array."""
    ...
    pcm_float = np.sign(expanded) * (1.0 / _MU) * ((1.0 + _MU)**np.abs(expanded) - 1.0)
    # Scale back to 16-bit integer range and convert type
    pcm_int16 = (pcm_float * 32768.0).astype(np.int16)
    return pcm_int16.tobytes()
# --- End Mu-law functions ---

# List of keys that are expected to contain binary data
BINARY_KEYS = ['data', 'chunk', 'frame']

def safe_serialize(payload):
    """Safely serializes a payload to JSON, encoding binary fields."""
    try:
        ...
        logging.error(f"Safe-serialize error: {e}. Payload: {payload.get('type', 'N/A')}")
        return b"{}" # Return empty JSON object on failure

def safe_deserialize(data):
    """Safely deserializes JSON data, decoding binary fields."""
    try:
        ...
        logging.error(f"Safe-deserialize error: {e}. Data: {data[:50]}...")
...
# --- Audio Mixing Configuration ( to original) ---
...
# --- Video Buffering Config ( to original) ---
VIDEO_BUFFER_TIMEOUT_SEC = 2
# --------------------------------

# --- Style Configuration (Modern Dark Theme) ---
BG_COLOR = "#1E1E1E"        # Very dark grey
FRAME_BG = "#2D2D2D"    # Lighter grey for frames/dividers
LIST_BG = "#252525"     # Background for lists and logs
LOG_BG = "#252525"      # Same as list
FG_COLOR = "#F1F1F1"        # Off-white text
ACCENT_COLOR = "#0E63F4"  # Professional blue (Zoom-like)
ACCENT_DARK = "#0A4ABF"   # Darker blue for hover
BTN_SUCCESS = "#28a745"   # Green (unchanged)
FG_MUTED = "#9A9A9A"       # Muted text color for info
# ------------------------------------------------------
...
class ServerGUI(QWidget):
    def __init__(self):
        ...
        self.udp_packet_queue = queue.Queue()
        self.audio_buffers = {} 
    ...
    def start_udp_server(self):
        ...
        try:
            ...
            self.udp_process_thread = threading.Thread(target=self.process_udp_queue, name="UDP-Process", daemon=True)
            ...
            try:
                ...
                with self.client_lock:
                    if sender_addr not in self.known_udp_addrs:
                        continue 
                self.udp_packet_queue.put((data, sender_addr))
            except OSError as e:
                if self.running.is_set():
